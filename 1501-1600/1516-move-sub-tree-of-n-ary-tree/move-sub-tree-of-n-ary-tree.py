...
class Solution:
    def moveSubTree(self, root: 'Node', p: 'Node', q: 'Node') -> 'Node':
        ...
        p_parent = self.dfs_parent(dummy, p) # Find p's parent (guaranteed to exist now because of the dummy node)
        q_in_p = self.dfs_parent(p, q) # Check if q is in p's subtree
        
        p_index = p_parent.children.index(p) # Get p's original index in p's parent
        p_parent.children.pop(p_index) # Remove p from p's parent. Again, guaranteed to succeed because of the dummy node
